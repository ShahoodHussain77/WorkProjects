...
class Notice extends Component {
    ...
    async componentWillMount() {
        ...
        if( group_id && group_id != '' && undefined != group_id) {
            url = `${Constants.BASE_URL}get_group_notices.php`;
        } else {
            url = `${Constants.BASE_URL}get_all_employer_notices.php`;
        }
        fetch(url,{
            method: 'POST',
            headers: {
        ...
        .then(( responseJson ) => {
            if( responseJson || responseJson != '' || responseJson != undefined || responseJson != 'no notice found' ) {
